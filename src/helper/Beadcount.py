    ...
    for img in img_list[0:]:
        if img is not None:
            ...
            if shape[0] == shape[1] and shape[0] != 0:
                ...
                resized_img = tools.Image_Data_Convert(
                    resized_img, 0, 255, np.uint8)

                if np.mean(img) > 200:
                    normed_img = tools.Brightfield_Norm(
                        resized_img, indv_img_shape)
                else:
                ...
                normed_img_new = cv2.cvtColor(
                    normed_img_new, cv2.COLOR_BGR2GRAY)
                thresholded = cv2.adaptiveThreshold(
                    normed_img_new, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)
                ...
                mask = cv2.circle(
                    mask, (50, 50), crop_detection_reduce, (50, 50, 50), -1)
                mask = tools.Image_Data_Convert(mask, 0, 255, np.uint8)
                bitwise_and_img = cv2.bitwise_and(dialation, mask, mask=None)

                circles = cv2.HoughCircles(
                    bitwise_and_img, cv2.HOUGH_GRADIENT, 1, 30, param1=12, param2=12, minRadius=25, maxRadius=60)
