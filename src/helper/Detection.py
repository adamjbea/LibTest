        ...
        if c[0] > 125 and c[0] < 2475:
            if c[1] > 125 and c[1] < 1825:
                ...
                if c is not None:
                    ...
                    crop = img_copy[c[1]-crop_bead_increase:c[1]+crop_bead_increase,
                                    c[0]-crop_bead_increase:c[0]+crop_bead_increase]
                    mask = np.zeros(crop.shape)
                    mask = cv2.circle(
                        mask, (crop_bead_increase, crop_bead_increase), crop_bead_increase, (255, 255, 255), -1)

                    final_im = crop/mask
                    final_im[final_im == inf] = 0

                    crop_c = img_clean[c[1]-crop_bead_increase:c[1]+crop_bead_increase,
                                       c[0]-crop_bead_increase:c[0]+crop_bead_increase]
                    final_im_c = crop_c/mask
                    final_im_c[final_im_c == inf] = 0

                    if np.count_nonzero(final_im) != 0:
                        final_im = tools.Image_Data_Convert(
                            final_im, 0, 255, np.uint8)
                        hsv = cv2.cvtColor(final_im, cv2.COLOR_BGR2HSV)
                        lower_red = np.array([0, 200, 0])
                        upper_red = np.array([255, 255, 255])
                        mask = cv2.inRange(hsv, lower_red, upper_red)
                        res = cv2.bitwise_and(
                            final_im_c, final_im_c, mask=mask)
                        mean_outline = np.mean(res)
                        average = res[np.nonzero(res)].mean()
                        decision_img = final_im_c - res
                        average_img = decision_img[np.nonzero(
                            decision_img)].mean()
    ...
    for c in circles_grey[0, :]:
        ...
        if c is not None:
            ...
            crop = img_copy[c[1]-crop_bead_increase:c[1]+crop_bead_increase,
                            c[0]-crop_bead_increase:c[0]+crop_bead_increase]
            mask = np.zeros(crop.shape)
            mask = cv2.circle(mask, (crop_bead_increase, crop_bead_increase),
                            crop_bead_increase, (255, 255, 255), -1)
...
def Circle_Finder(img: np.array, color: str) -> list:
    ...
    if color == 'green':
        ...
        circles = cv2.HoughCircles(
            mask, cv2.HOUGH_GRADIENT, 1, 30, param1=50, param2=12, minRadius=35, maxRadius=60)
        ...
        circles = cv2.HoughCircles(
            mask, cv2.HOUGH_GRADIENT, 1, 30, param1=50, param2=10, minRadius=35, maxRadius=55)
    ...
    # adaptive grey circle detector - runs multiple detections and chooses the best
        ...
        try:
            circles_b = cv2.HoughCircles(
                img_bw, cv2.HOUGH_GRADIENT, 1, 30, param1=60, param2=30, minRadius=30, maxRadius=50)
            num_b = len(circles_b[0])
        except:
            pass
        try:
            circles_c = cv2.HoughCircles(
                img_bw, cv2.HOUGH_GRADIENT, 1, 50, param1=35, param2=40, minRadius=70, maxRadius=120)
...
def get_contours(image):
    ...
    threshold = cv2.threshold(
        c_image, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
...
def Flour_Blob(image, color, threshold=0):
    ...
    pruned_contour_list, intensity_list, lst_display = contour_analysis(
        image, rough_contour_list, threshold, color)
