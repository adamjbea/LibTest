...
# For each column, pass a list where the first entry is the column's name.
def Write_CSV(output_folder, Run_ID, CSV_Output_Browser, *args):

    filename = output_folder + "/" + \
        str(datetime.date.today()) + "_" + Run_ID + "_"
    ...
    try:
        ...
        with open(f"{filename}{i}.csv", 'w') as output:
            # DictWriter can also be used if you pass in a dictionary with column name, row name for all entries.
    ...
    if size == (1920, 2560):
        ...
        if avg_pixel < 180:
            normalized_img = cv2.normalize(
                img,  norm_img, 70, 300, cv2.NORM_MINMAX)
...
def Change_Image_Size(shape, image):
    maxWidth = shape[0]
    maxHeight = shape[1]
