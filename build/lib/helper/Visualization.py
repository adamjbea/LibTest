...
def FluorescenceNormalize(blue, bw, green):
    ...
    image4 = visual.changeImageSize(2560, 1920, blue)
    image5 = visual.changeImageSize(2560, 1920, green)
...
def Histogram_Grey(img_bw):
    ...
    plt.hist(img_bw.ravel(), 100, [20, 200],
            density=True, alpha=0.3, color="black")
...
def Display_Pictures(print_pictures, range_list, name_list):
    ...
    if size_wanted != 0:
        ...
        for i in range(10000)[range_list[0]:range_list[1]]:
            plt.subplot(columns, row, j +
                        1), plt.imshow(print_pictures[i], 'gray')
