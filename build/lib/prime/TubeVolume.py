...
# pixels/mm (Fixed for EVOS objective), used to calculate uL volume
PIX_MM = 35
...
# Set image min and max gray to be [0] and [1] quantile
GRAY_THRESH = [.4, .95]
# Intensity pixel difference defining a tube edge (threshold of derivative)
EDGE_THRESH = 15.0/255
# Median filter size for smoothing emulsion derivative (pixels)
EMUL_SMOOTH = 21
...
# Pixels from top below which emulsion/tube bend,tube bottom might start
OFFSETS = [50, 20, 450]
...
def calc_profiles(img):
    ...
    tube = {}
    ny = img.shape[0]
    # Tubes' left coordinates at each height
    tube['left'] = np.zeros((ny, 8), dtype=int)
    # Tubes' right coordinates at each height
    tube['right'] = np.zeros((ny, 8), dtype=int)
    # Tubes' widths at each height
    tube['width'] = np.zeros((ny, 8), dtype=int)
    # Tubes' median image values at each height
    tube['val'] = np.zeros((ny, 8))
    ...
    for i in range(8):
        ...
        for y in range(ny):
            ...
            if (len(left_edge) > 0) and (len(right_edge) > 0) and left_edge[0] < right_edge[-1]:
                tube['left'][y, i] = left_edge[0] + \
                    TUBE_PITCH*i  # Left coordinate
                tube['right'][y, i] = right_edge[-1] + \
                    TUBE_PITCH*i  # Right coordinate
                tube['width'][y, i] = tube['right'][y, i] - \
                    tube['left'][y, i]  # Width
                tube['val'][y, i] = np.median(
                    img[y, tube['left'][y, i]:tube['right'][y, i]])  # Median value

    # Clean-up and scale median tube value
    tube['val'] = medfilt(np.nan_to_num(tube['val']), [
                        EMUL_SMOOTH, 1])  # Smooth emulsion values
    tube['val'] = np.divide(tube['val'], np.max(
        tube['val'][:OFFSETS[2]+5, :], axis=0))  # Scale to maximum value (0-1)

    # Clean-up and scale tube width
    tube['width'] = convolve2d(tube['width'], np.ones(
        (WIDTH_SMOOTH, 1))/WIDTH_SMOOTH)  # Smooth widths
    tube['width'] = np.divide(tube['width'], np.max(
        tube['width'], axis=0))  # Scale to maximum width (0-1)
...
def calc_metrics(tube):
    ...
    locs['Tube_Bend'] = [np.where(tube['width'][OFFSETS[1]:, i] < .95)[
        0][0]+OFFSETS[1] for i in range(8)]

    # Tube bottom is where width is almost zero
    locs['Tube_Bot'] = [np.where(tube['width'][OFFSETS[2]:, i] < .1)[
        0][0]+OFFSETS[2]-10 for i in range(8)]
...
def plot_volumes(img0, tube, locs, vol, file):
    ...
    for i in range(8):
        ...
        # Display title
        plt.title('Total Emulsion: %3.0fuL, Total Oil: %3.0fuL' %
                (np.sum(vol[:, 0]), np.sum(vol[:, 1])), fontsize=16)
