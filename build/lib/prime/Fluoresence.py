...
# Minimum diameter to perimeter ratio (~eccentricity). For circle, diam/perim = 1/pi = 0.32
MIN_DIAM_PERIM = .25
# Colors that can be loaded from files
VALID_COLORS = ['RGB', 'BF', 'DAPI', 'GFP', 'RFP', 'CY5']
# Fluorescence channels that can be displayed
VALID_FLUOR = ['BF', 'DAPI', 'GFP', 'RFP', 'CY5']
...
def Fluorescence_Controller(input_dire, dire):
    ...
    if identifier_and_images != []:
        container = Read_Decide_Analyze(
            identifier_and_images, dire, input_dire)
        return container
    ...
    for color in imgs.keys():
        if color in VALID_FLUOR:
            ...
            if len(imgs[color].shape) == 3:
                imgs[color] = tools.Image_Data_Convert(
                    imgs[color], 0, 255, np.uint8)
...
def find_drops(imgs):
    ...
    for color in imgs.keys():
        if color in VALID_FLUOR:
            drops[color] = labeled_comprehension(
                imgs[color], lbl, lbls, np.mean, float, 0)
            if 'RGB' in imgs.keys():
    ...
    pixel_area = labeled_comprehension(
        imgs['BW'] > -1, lbl, lbls, np.sum, float, 0)
    drops['Radius'] = np.sqrt(pixel_area/np.pi)
    drops['Diameter'] = (np.sqrt(pixel_area/np.pi) *
                         2+DROP_BORDER) * CONVERSION_4X
    drops['perimeter'] = labeled_comprehension(
        imgs['BW'] > -1, lbl_borders, lbls, np.sum, float, 0) * CONVERSION_4X

    mx, my = np.meshgrid(
        np.arange(imgs['BW'].shape[1]), np.arange(imgs['BW'].shape[0]))
    ...
    good = (np.divide(drops['Diameter'], drops['perimeter']+20)
            > MIN_DIAM_PERIM) & (drops['Diameter'] > MIN_DIAM)
    drops['good'] = good
    ...
    colors = [c for c in colors if c in [
        'DAPI', 'GFP', 'RFP', 'CY5', 'Diameter']]
    nc = len([c for c in colors if c in [
            'DAPI', 'GFP', 'RFP', 'CY5', 'Diameter']])
    ...
    for i in range(nc):
        ...
        drops['color_thresh'][i] = SCATTER_THRESH * \
            np.max(di)+(1-SCATTER_THRESH)*np.min(di)
...
def plot_figures(input_dire, name, imgs, drops):
    ...
    for i in range(nc):
        ...
        if colors[i] == 'Diameter':
            ...
            # Mask fluorescence by BW
            plt.imshow(np.multiply(imgs[colors[i]],
                    imgs['BW']), cmap=ListedColormap(vals))
            di = drops[colors[i]]
        ...
        if isinstance(outline_color, str):
            plt.scatter(x=XX+np.multiply(np.cos(TH), RR), y=YY +
                        np.multiply(np.sin(TH), RR), c=outline_color, s=.5)
        # else:
        ...
        plt.title(colors[i] + ' (Masked)' if colors[i]
                != 'Diameter' else 'BF', fontsize=20)
    ...
    plt.loglog(drops['Diameter']+.05, np.divide(drops['Diameter'],
            drops['perimeter']+20), '.', markersize=1)
    plt.plot(plt.xlim(), [MIN_DIAM_PERIM, MIN_DIAM_PERIM], 'k--')
    plt.plot([MIN_DIAM, MIN_DIAM], plt.ylim(), 'k--')
    plt.text(plt.xlim()[0]*1.5, (MIN_DIAM_PERIM+plt.ylim()[0])/2,
            'Small, Not Round\n(BAD)', color=[1, 0, 0], va='center', ha='left')
    plt.text(plt.xlim()[1]/1.5, (MIN_DIAM_PERIM+plt.ylim()[0])/2,
            'Large, Not Round\n(BAD)', color=[1, 0, 0], va='center', ha='right')
    plt.text(plt.xlim()[0]*1.5, (MIN_DIAM_PERIM+plt.ylim()[1])/2,
            'Small, Round\n(BAD)', color=[1, 0, 0], va='center', ha='left')
    plt.text(plt.xlim()[1]/1.5, (MIN_DIAM_PERIM+plt.ylim()[1])/2,
    ...
    for i in range(nc):
        for j in range(i+1, nc):
            ...
            # Diameter threshold is 76um
            xth = drops['color_thresh'][i] if colors[i] != 'Diameter' else 76
            ...
            plt.xlabel(colors[i]+' (RFU)' if colors[i] !=
                    'Diameter' else 'Diameter (um)', fontsize=14)
            plt.ylabel(colors[j]+' (RFU)' if colors[j] !=
                    'Diameter' else 'Diameter (um)', fontsize=14)
            ...
            for m in range(2):
                ...
                if colors[i] != 'Diameter':
                    for k in range(2):
                        kk = di*(k-.5) <= xth*(k-.5)
                        if np.sum(kk & mm) > 0:
                            plt.text(np.quantile(di[kk & mm], .8), np.quantile(
                                dj[kk & mm], .8), str(np.sum([kk & mm])), fontsize=16)
                else:
                    if np.sum(mm) > 0:
                        plt.text(np.quantile(di, .8), np.quantile(
                            dj[mm], .8), str(np.sum([mm])), fontsize=16)

            # Plot x histogram
            bins = drops['bins']
            hists = drops['hists']
            xlim = plt.xlim()
            plt.plot((hists[j]/np.max(hists[j]))*np.diff(xlim)
                     * .15+xlim[0], bins[j][:-1], 'k')
            plt.xlim(xlim)

            # Plot y histogram
            ylim = plt.ylim()
            ylim = [ylim[0]-np.diff(ylim)*.2, ylim[1]]
            plt.plot(bins[i][:-1], hists[i]/np.max(hists[i])
                     * np.diff(ylim)*.15+ylim[0], 'k')
            plt.ylim(ylim)
...
def fluor_metrics(drops, name):
    ...
    for i in range(nc):
        ...
        metrics[colors[i] +
                '+'] = [np.sum((d > drops['color_thresh'][i]))/len(d)]
    ...
    metrics['Area>2XMed'] = np.sum(
        np.square(diam[np.square(diam) > 2*med**2]))/np.sum(np.square(diam))
        ...
        for key in uploaded:
            ...
            for img_set in identifier_and_images:
                if img_identity == img_set[0]:
                    identifier_and_images[k][1].append(
            ...
            if found_identity == False:
                identifier_and_images.append(
                    [img_identity, [[img, color, image_name]]])
    ...
    for image_set in identifier_and_images:
        ...
        if number_pictures <= 4:
            ...
            if img_set[0].any() and img_set[2].any() and img_set[3].any():
                imgs = load_images(dirpath=Directory+'/',
                                BF=name_BF, GFP=name_GFP, DAPI=name_DAPI)
            elif img_set[0].any() and img_set[1].any() and img_set[3].any():
                imgs = load_images(dirpath=Directory+'/',
                                BF=name_BF, GFP=name_GFP, CY5=name_CY5)
            elif img_set[0].any() and img_set[1].any():
                imgs = load_images(dirpath=Directory+'/',
                                BF=name_BF, CY5=name_CY5)
            elif img_set[0].any() and img_set[3].any():
                imgs = load_images(dirpath=Directory+'/',
                                BF=name_BF, GFP=name_GFP)
            elif img_set[0].any() and img_set[2].any():
                imgs = load_images(dirpath=Directory+'/',
                                BF=name_BF, DAPI=name_DAPI)
            ...
            try:
                ...
                final_data = final_data.append(
                    fluor_metrics(drops, name), ignore_index=True)
